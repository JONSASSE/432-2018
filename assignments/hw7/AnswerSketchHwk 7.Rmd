---
title: "432 Assignment 7 Answer Sketch"
author: "432 Staff"
date: "Due April 24, 2018 at 5:00 pm"
output:
  html_document:
    df_print: paged
geometry: margin=1in
...
library(skimr);library(rms); library(ggrepel);library(readxl);library(survival);library(OIsurv); library(survminer); library(broom);library(tidyverse)
```
...
- Remember to **ask for help** if you are stuck, first by Googling, or visiting the wonderful cheat sheets under the Help menu on the top bar of R Studio, and then by contacting `431-help at case dot edu` or visiting TA office hours.
...
You will make a meaningful plot, but since is playoffs season here is a plot of the Cleveland Cavaliers winning percentage throughout history. [Data](https://www.basketball-reference.com/teams/CLE/)
...
##Using coxph
```{r}
mod_A1 <-with(umaru, coxph(Surv(time, censor==1) ~ ivhx + treat + race +  ivhx*treat + rcs(ndrugtx, 3) + beck + hercoc))

summary(mod_A1)
```

The model doesn't actually fit very well. Neither the `ivhx` term, nor its interaction with `treat` is of much use, which is a bit of a surprise. The hazard ratio estimate for the "Short" duration `treat`ment is 1.30, but has a 95\% CI of (0.96, 1.76) after adjusting for these predictors.
...
```{r K-M estimated survival functions for the two treatment groups}
...
- So our estimates are generated; now, we plot them. Treatment B appears to have a higher probability of remission-free survival, especially past 30 or so days.

```{r km plot for remission comparison by treatment}
...
Though treatment B appears to have a better survival profile, at least by the log rank test, the difference between the two treatments' survival functions does not reach the level of statistical significance. This is certainly related to the small sample size in each treatment group.
