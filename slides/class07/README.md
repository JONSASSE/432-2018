...
- [The slides](https://github.com/THOMASELOVE/432-2018/tree/master/slides/class07) as **revised** to fix the typos unearthed in class on slides 19 and 38, and 
- The [audio files](https://github.com/THOMASELOVE/432-2018/tree/master/slides/class07).
...
4. **What's coming up?** (A tentative plan)
    ...
    - After that: cleanup loose ends, discuss linear regression issues raised in Chapter 11, then, in some order ...
...
2. **Are models ever correct?** Is there such a thing as a "true" model, or a "correct" model. Or are models just relatively better or worse, in most cases?
...
3. **How do I pick the best model/strategy?** A related question to that one was "When adjusted R, Cp, AIC, and BIC all say differently, what should I go with, in terms of choosing the best subset?" Another was "When should I use one stepwise procedure instead of another? (Forward vs. Backward vs. Allen-Cady, for example.)
...
4. **Small R-square.** If all of the R^2 values are small, should I still use linear regression models to predict the outcome?
...
5. **Motivating Transformations** How do you know if you need to transform a predictor variable?
...
6. **Degrees of Freedom** I don’t have a great understanding of degrees of freedom and how these are important to our models. Help?
...
7. **ANOVA with no data** What is the best way to deal with a ANOVA when one of the category levels has a zero sample size, perhaps in an interaction.
...
8. **On "borrowing" code** Does one ever get to the point where they can enter in R code without having to "crib" other people's code?
...
9. **Underfitting or overfitting** What does it mean to over-fit or under-fit a model?
...
10. **Interactions with multiple variables** If you think `sex` interacts with both `race` and `treatment` in predicting an `outcome`, how could you fit a model to indicate that?
    - Try something like `lm(outcome ~ age + sex*race + sex*treatment + other variables, data = dataset)`
...
11. **Number of folds in cross-validation** In cross-validation of a linear regression model, what do you recommend in terms of the number of folds for our validation? (In other words, what do you recommend we use for k in k-fold cross validation?)
...
12. **Making leaps tidy** Do you anticipate the leaps package eventually being absorbed into the tidyverse?
...
13. **Centering and rescaling** When is centering and rescaling a predictor most often called for?
...
14. **Project Question** Is it possible to work on Project 1 or Project 2 for 432 with a partner?
...
15. **Project Question** Will you help me find a data set for Project 1 or 2?
...
16. **On Getting Better at this stuff** What do you recommend I read to learn more about how to most effectively do "type of analysis X" or how to choose between "approach A and approach B" in doing "type of analysis X"?
