...
# The PTSD study (See Class 07 Slides)
...
## `maleptsd %>% select(-id, -ptsd.raw) %>% skim()`
...
Step | AIC before step | Decision | Resulting AIC
---: | ----------: | ------------- | -----------:
1 | 28.41 | drop `aff` | 26.80
2 | 26.80 | drop `contr` | 25.42
3 | 25.42 | drop `over5` | 24.54
4 | 24.54 | drop `posit` | 23.87
5 | 23.87 | drop `over2` | 23.75
6 | 23.75 | stop
...
```{r}
st.ptsd <- maleptsd %>% select(-id, -ptsd.raw) %>% 
    scale() %>% as.data.frame()
...
```{r, eval = F}
temp_res <- data_frame(observed = maleptsd$ptsd,
                       fitted = fits.cv$fit,
...
Sure. The `glmnet` package is an interesting way to do several sets of model-building activities when the number of predictors is much larger than the sample size, especially if the predictors can be rescaled so as to avoid collinearity. We may return to this later in the term. An advantage of `glmnet` is that tidiers from `broom` are available.

Check out, for instance, the elastic net (which bridges the gap between the lasso and ridge regression), and its performance comparison for a simulated study at https://www4.stat.ncsu.edu/~post/josh/LASSO_Ridge_Elastic_Net_-_Examples.html

Or take a look at the `glmnet` package vignette at http://127.0.0.1:36989/help/library/glmnet/doc/glmnet_beta.pdf
