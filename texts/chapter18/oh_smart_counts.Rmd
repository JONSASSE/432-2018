---
title: "Modeling a Count Outcome in Ohio SMART"
...
## note that I will also make use of the pscl package
## which I didn't load here because it conflicts with 
## the countreg package, and I need countreg first
...
```{r import}
smart_oh <- read.csv("smart_ohio.csv") %>% tbl_df
```

Note that I will also use the `MASS` package, but won't install it because it conflicts with the `tidyverse` in terms of the `select` function, which is annoying.
...
## Dropping the subject with an unreasonably large `bmi`
...
## Revised (Final) Scatterplot Matrix

Now, here's the revised scatterplot matrix for those `r nrow(sm_oh_A_young)` subjects, now using the centered `bmi` data captured in the `bmi_c` variable.
...
- The `bmi_c` coefficient, 0.15, indicates that for each additional kg/m^2^ of BMI, while holding `smoke100` constant, the predicted `physhealth` value increases by 0.15 day.
...
- The intercept, 0.48, is the predicted logarithm of (`physhealth` + 1) (in days) for a subject with average BMI who has not smoked 100 cigarettes or more.
    - We can exponentiate to see that the prediction for (`physhealth` + 1) here is `exp(0.48)` = `r round(exp(0.48),2)` so the predicted `physhealth` for a subject with average BMI who has not smoked 100 cigarettes is `r round(exp(0.48),2) - 1` days.
- The `bmi_c` coefficient, 0.15, indicates that for each additional kg/m^2^ of BMI, while holding `smoke100` constant, the predicted logarithm of (`physhealth` + 1) increases by 0.026
...
- The constant term, 0.63, gives us the intercept of the regression - the prediction if `smoke100 = 0` and `bmi_c = 0`. In this case, because we've centered BMI, it implies that `exp(0.63)` = `r round(exp(0.63),2)` is the predicted days of poor `physhealth` for a non-smoker with average BMI. 
- The coefficient of `bmi_c`, 0.043, is the expected difference in count of poor `physhealth` days (on the log scale) for each additional kg/m^2^ of body mass index. The expected multiplicative *increase* is $e^{0.043}$ = `r round(exp(0.043),3)`, corresponding to a 4.4% difference in the count.
- The coefficient of `smoke100`, 0.71, tells us that the predictive difference between those who have and who have not smoked 100 cigarettes can be found by multiplying the `physhealth` count by exp(0.71) = `r round(exp(0.71),2)`, yielding essentially a doubling of the `physhealth` count.
...
The sum of squared standardized residuals $\sum z^2$ = `r sum(z^2)`. The estimated overdispersion factor is `r sum(z^2)`/`r n-k` = `r round(sum(z^2)/ (n - k),1)`, and the p value here is 0, indicating that the probability is essentially zero that a random variable from a $\chi^2$ distribution with (n - k) = 1633 degrees of freedom would be as large as `r sum(z^2)`. 

In summary, the polyps counts are overdispersed by a factor of 15.499, which is enormous (even a factor of 2 would be considered large) and also highly statistically significant. The basic correction for overdisperson is to multiply all regression standard errors by $\sqrt{15.499}$ = `r round(sqrt(15.499),2)`. 
...
- The constant term, 0.61, gives us the intercept of the regression - the prediction if `smoke100 = 0` and `bmi_c = 0`. In this case, because we've centered BMI, it implies that `exp(0.61)` = `r round(exp(0.61),2)` is the predicted days of poor `physhealth` for a non-smoker with average BMI. 
- The coefficient of `bmi_c`, 0.043, is the expected difference in count of poor `physhealth` days (on the log scale) for each additional kg/m^2^ of body mass index. The expected multiplicative *increase* is $e^{0.043}$ = `r round(exp(0.043),3)`, corresponding to a 4.4% difference in the count.
- The coefficient of `smoke100`, 0.756, tells us that the predictive difference between those who have and who have not smoked 100 cigarettes can be found by multiplying the `physhealth` count by exp(0.756) = `r round(exp(0.756),2)`, yielding essentially a doubling of the `physhealth` count.
...
```
and there's a $\theta$ term, which is estimated as `exp(-1.023)` or `r round(exp(-1.023),3)`, and this negative binomial regression model may also produce some additional zero count estimates.
...
## Fitting a Different Hurdle Model
...
- the coefficient labeled `(Intercept):2` is a statistic we can use after we exponentiate it, as follows:
    - here `(Intercept):2` = 2.82, and exp(2.82) = `r exp(2.82)`, which is analogous to the square root of the residual variance in OLS regression, which is summarized for our OLS model as `Residual standard error: 6.609`.

The residuals and fitted values from the tobit model can be stored and then summarized in several ways:

```{r}
sm_oh_A_young1 <- sm_oh_A_young %>%
    mutate(fitted_tob1 = fitted(mod_tob1),
           resid_tob1 = resid(mod_tob1, type = "response"))
```

Here is a plot of residuals vs. fitted values.

```{r}
ggplot(sm_oh_A_young1, aes(x = fitted_tob1, y = resid_tob1)) +
    geom_point() +
...
```{r}
qqnorm(sm_oh_A_young1$resid_tob1)
```

We can look at the correlation between the actual and model-predicted values of `physhealth` using the tobit model with:

```{r}
# correlation
(r_tob1 <- with(sm_oh_A_young1, cor(fitted_tob1, physhealth)))

# variance accounted for
r_tob1^2
```

and here's the plot comparing the actual and predicted values under the tobit model. Note that the model never predicts a subject to have an underlying `physhealth` worse than about 12 (remember that larger numbers are worse here.) In fact, the model predicts subjects to have an underlying `physhealth` ranging from `r round(min(sm_oh_A_young1$fitted_tob1),1)` to `r round(max(sm_oh_A_young1$fitted_tob1),1)` days.

```{r}
ggplot(sm_oh_A_young1, aes(x = fitted_tob1, y = physhealth)) +
    geom_point()
```
...
Suppose we decide to add `alcdays` and `incomegroup` to our models. Now what happens?

## A Bigger Scatterplot Matrix

```{r}
GGally::ggpairs(sm_oh_A_young %>% 
                    select(bmi_c, smoke100, alcdays, 
                           incomegroup, physhealth))
```

## OLS approach

```{r}
mod_ols2 <- lm(physhealth ~ sleephrs + bmi_c + smoke100 + 
                   alcdays + incomegroup, 
               data = sm_oh_A_young)
...
```{r}
mod_tob2 <- vglm(physhealth ~ sleephrs + bmi_c + smoke100 + 
                   alcdays + incomegroup, 
                 tobit(Lower = 0, Upper = 30),
                 data = sm_oh_A_young)
...
In our second tobit model, 

- a one-unit increase in `bmi_c` is now associated with a 0.39 day increase in the predicted value of `physhealth`, holding everything else constant
- the household income groups appear to split between the low group and a higher ($25,000 or more per year) group in terms of impact on `physhealth` with higher income associated with better (lower) `physhealth`.
- the coefficient labeled `(Intercept):1` is again the intercept for the model. The value is 9.3, which is in the range of `physhealth` values we observed.
- the coefficient labeled `(Intercept):2` is a statistic we can use after we exponentiate it, as follows:
    - here `(Intercept):2` = 2.75, and exp(2.75) = `r exp(2.75)`, which is analogous to the square root of the residual variance in OLS regression, which is summarized for our OLS model 2 as `Residual standard error: 6.336`.

Summarizing the tobit 2 residuals and fitted values:

```{r}
sm_oh_A_young2 <- sm_oh_A_young %>%
    mutate(fitted_tob2 = fitted(mod_tob2),
           resid_tob2 = resid(mod_tob2, type = "response"))
```

Here is a plot of residuals vs. fitted values.

```{r}
ggplot(sm_oh_A_young2, aes(x = fitted_tob2, y = resid_tob2)) +
    geom_point() +
    labs(title = "Residuals vs. Fitted Values for Tobit 2")
```

Here is a normal Q-Q plot of the Tobit Model 2 residuals.

```{r}
qqnorm(sm_oh_A_young2$resid_tob2)
```

And, again, we can look at the correlation between the actual and model-predicted values of `physhealth` using the tobit model with:

```{r}
# correlation
(r_tob2 <- with(sm_oh_A_young2, cor(fitted_tob2, physhealth)))

# variance accounted for
r_tob2^2
```

Finally, here's the plot comparing the actual and predicted values under the second tobit model. Note that this new model predicts subjects to have an underlying `physhealth` ranging from `r round(min(sm_oh_A_young2$fitted_tob2),1)` to `r round(max(sm_oh_A_young2$fitted_tob2),1)` days.


```{r}
ggplot(sm_oh_A_young2, aes(x = fitted_tob2, y = physhealth)) +
    geom_point()
```