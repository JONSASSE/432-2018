---
title: "Chapter 18: Modeling a Count Outcome in Ohio SMART"
...
which is truncated to produce only estimates greater than zero, with $\theta$ estimated as `exp(-1.123)` or `r round(exp(-1.123), 3)`.
...
Note that the model never predicts a subject to have an underlying `physhealth` worse than about 12 (remember that larger numbers are worse here.) In fact, the model predicts subjects to have an underlying `physhealth` ranging from `r round(min(sm_tob1$fitted_tob1),1)` to `r round(max(sm_tob1$fitted_tob1),1)` days.
