---
title: "BRFSS Data (2016) and the SMART program"
...
Let's use the `read_xpt` function from the `haven` package to bring in the SAS XPT data file that is provided by CDC.

```{r ingest_raw}
smart.raw <- read_xpt("MMSA2016.xpt") %>% tbl_df

dim(smart.raw)
```

Nationwide, we have `r nrow(smart.raw)` observations on `r ncol(smart.raw)` variables.
...
`WTKG3` is computed by CDC, as the respondent's weight in kilgrams with two implied decimal places. We calculate the actual weight in kg, with the following:
...
- BMI of 30 and higher indicatdes obesity

```{r eda_bmigroup}
smart.raw$bmigroup <- Hmisc::cut2(smart.raw$bmi, cuts = c(18.5, 25.0, 30.0))
...
write_csv(smart_ohio, "smart_ohio.csv")
```
...
write_csv(smart.cle, "smartcle.csv")
```
...
write_csv(smart.cle1, "smartcle1.csv")
```

```{r}
modA <- lm(physhealth ~ bmi + female + internet30 + genhealth + exerany + sleephrs + alcdays, data = smart.cle1)

summary(modA)

modB <- lm(menthealth ~ bmi + female + internet30 + genhealth + exerany + sleephrs + alcdays, data = smart.cle1)

summary(modB)

modC <- lm(poorhealth ~ bmi + female + internet30 + genhealth + exerany + sleephrs + alcdays, data = smart.cle1)
